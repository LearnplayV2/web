	...
	return (
		<div css={Styles.form} ref={ref}>
			{Array.from({ length: count }).map((e, i) => (
				<div className="row">
					{i == count - 1 && i != 0 && (
						<div className="btn bg danger" onClick={() => setCount((state) => state - 1)}>
					...
					{i == count - 1 && (
						<div className="btn bg success" onClick={() => setCount((state) => state + 1)}>
							<MdAdd title="Adicionar linha" />
						</div>
					)}
          <div className="mobile-btn">
					{(i == count - 1 && i != 0) && (
            <div className="bg danger" onClick={() => setCount((state) => state - 1)}>
              <FiTrash title="Deletar linha" />
            </div>
          )}
          </div>
